...
export const createRepository = (kvPool, schemaName, plugins = []) => {
    ...
    const save = async (tenantId, data, { version = null, atomic: externalAtomic } = {}) => {
        return kvPool.withConnection(async (kv) => {
            ...
            // Use passed version, or the one on the object if present
            ...
            // Strip _versionstamp before saving (it's metadata)
            ...
            // Return with new version? We don't have it unless we re-read or commit returns it (Deno KV commit returns versionstamp).
            // Deno KV commit returns { ok: true, versionstamp: ... }
            return Ok({ ...cleanData, _versionstamp: commitRes.versionstamp });
        });
    };
    ...
    const queryByIndex = async (tenantId, indexName, value, { limit = 10, cursor } = {}) => {
        return kvPool.withConnection(async (kv) => {
            ...
            const keys = [];
            // Map index result (key: ...idx... value: id)
