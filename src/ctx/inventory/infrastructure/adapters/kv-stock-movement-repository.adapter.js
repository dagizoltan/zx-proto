...
import { StockMovementSchema } from '../../../../ctx/inventory/infrastructure/persistence/schemas/stock-movement.schema.js';
import { stockMovementMapper } from '../../../../ctx/inventory/infrastructure/persistence/mappers/stock-movement.mapper.js';
...
export const createKVStockMovementRepository = (kvPool) => {
  ...
  return {
    save: async (tenantId, domainEntity) => {
      try {
        const persistenceModel = stockMovementMapper.toPersistence(domainEntity);
        const result = await baseRepo.save(tenantId, persistenceModel);
        if (isErr(result)) return result;
        return Ok(stockMovementMapper.toDomain(result.value));
      } catch (e) {
    ...
    findByIds: async (tenantId, ids) => {
      ...
      if (isErr(result)) return result;
      return Ok(stockMovementMapper.toDomainList(result.value));
    },
    ...
    list: async (tenantId, options) => {
      ...
      if (isErr(result)) return result;
      return Ok({ ...result.value, items: stockMovementMapper.toDomainList(result.value.items) });
    },
