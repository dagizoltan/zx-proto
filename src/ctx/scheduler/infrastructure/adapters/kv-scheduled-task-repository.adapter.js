...
import { ScheduledTaskSchema } from '../persistence/schemas/scheduled-task.schema.js';
import { scheduledTaskMapper } from '../persistence/mappers/scheduled-task.mapper.js';
...
export const createKVScheduledTaskRepository = (kvPool) => {
    ...
    return {
        save: async (tenantId, domainEntity) => {
            try {
                const persistenceModel = scheduledTaskMapper.toPersistence(domainEntity);
                // Ensure tenantId is passed correctly if mapper attached it
