...
import { PurchaseOrderSchema } from '../../../../ctx/procurement/infrastructure/persistence/schemas/purchase-order.schema.js';
import { purchaseOrderMapper } from '../../../../ctx/procurement/infrastructure/persistence/mappers/purchase-order.mapper.js';
...
export const createKVPurchaseOrderRepository = (kvPool) => {
  ...
  return {
    save: async (tenantId, domainEntity) => {
      try {
        const persistenceModel = purchaseOrderMapper.toPersistence(domainEntity);
        const result = await baseRepo.save(tenantId, persistenceModel);
        if (isErr(result)) return result;
        return Ok(purchaseOrderMapper.toDomain(result.value));
      } catch (e) {
    ...
    findByIds: async (tenantId, ids) => {
      ...
      if (isErr(result)) return result;
      return Ok(purchaseOrderMapper.toDomainList(result.value));
    },
