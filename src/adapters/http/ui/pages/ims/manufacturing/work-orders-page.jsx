  ...
  return (
    <div class="work-orders-page">
      <div class="page-header">
        <h1>Work Orders</h1>
        <a href="/ims/work-orders/new" class="btn btn-primary">Create WO</a>
      </div>
            ...
            <tbody>
              {workOrders.map(wo => (
                <tr>
                  ...
                  <td><a href={`/ims/work-orders/${wo.id}`}>{wo.code}</a></td>
                  <td><span class={`badge badge-${wo.status.toLowerCase()}`}>{wo.status}</span></td>
                  <td>{wo.productName || 'Unknown'}</td>
                  <td>{wo.quantity}</td>
                  <td>
                    <a href={`/ims/work-orders/${wo.id}`} class="btn btn-sm btn-secondary" style="margin-right: 0.25rem;">View</a>
