...
inventoryRoutes.get('/warehouses/new', async (c) => {
    const user = c.get('user');
...
inventoryRoutes.post('/warehouses', async (c) => {
    ...
    try {
        ...
        return c.redirect('/admin/inventory/warehouses'); // Normalized URL
    } catch (e) {
        return c.text(e.message, 400);
    }
});

inventoryRoutes.get('/warehouses/:id', async (c) => {
    const user = c.get('user');
    const tenantId = c.get('tenantId');
...
inventoryRoutes.get('/locations/new', async (c) => {
    const user = c.get('user');
    const tenantId = c.get('tenantId');
    const inventory = c.ctx.get('domain.inventory');
...
inventoryRoutes.post('/locations', async (c) => {
    const tenantId = c.get('tenantId');
    const inventory = c.ctx.get('domain.inventory');
    const body = await c.req.parseBody();
    ...
    try {
        ...
        return c.redirect('/admin/inventory/locations'); // Normalized URL
    } catch (e) {
        return c.text(e.message, 400);
    }
});

inventoryRoutes.get('/locations/:id', async (c) => {
    const user = c.get('user');
    const tenantId = c.get('tenantId');
