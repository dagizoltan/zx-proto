...
import { ProductDetailPage } from '../../pages/ims/product-detail-page.jsx'; // Note: Product Detail is often linked from Catalog
...
catalogRoutes.post('/categories', async (c) => {
    ...
    try {
        ...
        return c.redirect('/ims/catalog/categories'); // Normalized URL
    } catch (e) {
...
catalogRoutes.post('/price-lists', async (c) => {
    ...
    try {
        ...
        return c.redirect('/ims/catalog/price-lists'); // Normalized URL
    } catch (e) {
        return c.text(e.message, 400);
    }
});
